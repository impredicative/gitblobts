...
class Store:
    ...
    :param compression: name of a built-in or third-party importable module with ``compress`` and ``decompress``
        functions, e.g. :py:mod:`bz2`, :py:mod:`gzip`, :py:mod:`lzma`. Once established, this must not be changed for a
        given repository, failing which file corruption can result.
